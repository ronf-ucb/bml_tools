...
%  Andrew Pullin   2014-9-18   Update to deal with NaN's in tracking data
%
% Notes:
%   
% Usage:
...
fixINDerr =      @(x)regexprep(x,'-1.#IND0000','NaN');
...
%% locate frame info
framedata = regexp(content,'^frame,(\d*),([-\d.]*),(\d*),(.*)','tokens','once');
framedata = vertcat(framedata{:});

FrameIndices = framedata(:,1);
Timestamps   = framedata(:,2);
TrackCounts  = framedata(:,3);
rest         = framedata(:,4);
...
rest = cellfun(fixINDerr, rest, 'UniformOutput',false); %convet '-1.#IND0000' to NaN
...
for i=1:S.NumRigidBody
    ...
    if isempty(dataframes)
        %This handles the case that a trackable is defined, but then
        %tracked for no frames of the entire run
        S.Trackables(i).Position = [];
        S.Trackables(i).Quaternion = [];
        S.Trackables(i).Euler = [];
    else
